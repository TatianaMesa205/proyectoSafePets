...
class MascotasModel
{
    ...
    public static function mdlRegistrarMascota($nombre, $especie, $raza, $edad, $sexo, $tamano, $fecha_ingreso, $estado_salud, $estado, $descripcion, $imagen)
    {
        $mensaje = array();
        try {
            $objRespuesta = Conexion::conectar()->prepare("INSERT INTO mascotas(nombre, especie, raza, edad, sexo, tamano, fecha_ingreso, estado_salud, estado, descripcion, imagen) VALUES (:nombre, :especie, :raza, :edad, :sexo, :tamano, :fecha_ingreso, :estado_salud, :estado, :descripcion, :imagen)");
    ...
    public static function mldEditarMascota($id_mascotas, $nombre, $especie, $raza, $edad, $sexo, $tamano, $fecha_ingreso, $estado_salud, $estado, $descripcion, $imagen)
    {
        $mensaje = array();

        try {
            $objRespuesta = Conexion::conectar()->prepare("UPDATE mascotas SET nombre=:nombre, especie=:especie, raza=:raza, edad=:edad, sexo=:sexo, tamano=:tamano, fecha_ingreso=:fecha_ingreso, estado_salud=:estado_salud, estado=:estado, descripcion=:descripcion, imagen=:imagen WHERE id_mascotas=:id_mascotas");
            $objRespuesta->bindParam(":nombre", $nombre);
            $objRespuesta->bindParam(":especie", $especie);
            $objRespuesta->bindParam(":raza", $raza);
            $objRespuesta->bindParam(":edad", $edad);
